...
[(float(comp.loc), comp.weight) for comp in g.gmm]
    ...
    def update(self, obs):
        ...
        k = [np.reshape(dot(comp.cov, self.h.T)*s[index],[2,1])
        ...
        #dot(eye(len(k[index])) - dot(k[index], self.h), comp.cov)
        #comp.cov
    ...
    def prune(self, truncthresh=1e-5, mergethresh=0.01, maxcomponents=10):
    ...
    def extractstatesusingintegral(self, bias=1.0):
        ...
        """
        # print('int!!!!!!!!!!!!!!!!')
        # print(round(float(bias) * simplesum(comp.weight for comp in self.gmm)))
        ...
        # for iiii in range(2):
            ...
            numtoadd -= 1
        for x in items: print x.T
        # print('lengthlengthlengthlengthlengthlengthlengthlengthlengthlengthlength')
        # print(len(items))
        return items
