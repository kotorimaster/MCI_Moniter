...
t3 = threading.Thread(name="3", target=x3,args=(con,1,"3","com9"))
...
###################################################################################
        ...
        yuce_x1.append(km11.x[0, 0])
        yuce_y1.append(km21.x[0, 0])

        km12.z = np.array([rang12])
        km12.kf_update()
        km22.z = np.array([rang22])
        km22.kf_update()
        yuce_x2.append(km12.x[0, 0])
        yuce_y2.append(km22.x[0, 0])

        km13.z = np.array([rang13])
        km13.kf_update()
        km23.z = np.array([rang23])
        km23.kf_update()
        yuce_x3.append(km13.x[0, 0])
        yuce_y3.append(km23.x[0, 0])
