...
while (1)
    ...
    if mod(i,150)==0
        ...
        size(PureData)
        [br_s,hr_s]=respiration_multi2(PureData);   %%% wavelet
% 		[br_s,hr_s]=respiration_multi2_vncmd(PureData);   %%% VNCMD
        set(handles.text2,'string',num2str(br_s)); 
        set(handles.text1,'string',num2str(hr_s)); 
        toc
        pause(0.1)
        i=1;
        RawData=[];
