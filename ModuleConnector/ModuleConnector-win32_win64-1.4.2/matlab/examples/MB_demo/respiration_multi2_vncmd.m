...
function [br_v,hr_v,br_s,hr_s,t]=respiration_multi2_vncmd(Pure)
...
figure;
x=1/20:1/20:T/20;
subplot(211);plot(x,peaks'/156)
title("原始目标运动距离图");
xlabel("时间/s");
ylabel("距离/m");
% peaks=medfilt1(peaks,L1); %%中值滤波
peaks=kalmanfilter_3(peaks);
subplot(212);plot(x,peaks'/156)
title("MIK滤波后目标运动距离图");
xlabel("时间/s");
ylabel("距离/m");
adz=log(tops);
...
loc1=find(frqs>1&frqs<2);%>1
[val1,addrs1]=min(abs(frqs(loc1)-hr_v));
hr_s=frqs(loc1(addrs1))*60
loc2=find(frqs>0.01&frqs<0.5);%<0.5
[val2,addrs2]=min(abs(frqs(loc2)-br_v));
br_s=frqs(loc2(addrs2))*60
end
