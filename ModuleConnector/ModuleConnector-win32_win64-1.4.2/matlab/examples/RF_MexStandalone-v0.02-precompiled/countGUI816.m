function varargout = countGUI816(varargin)
% COUNTGUI62 MATLAB code for countGUI62.fig
%      COUNTGUI62, by itself, creates a new COUNTGUI62 or raises the existing
%      singleton*.
%
%      H = COUNTGUI62 returns the handle to a new COUNTGUI62 or the handle to
%      the existing singleton*.
%
%      COUNTGUI62('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in COUNTGUI62.M with the given input arguments.
%
%      COUNTGUI62('Property','Value',...) creates a new COUNTGUI62 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before countGUI62_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to countGUI62_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help countGUI62

% Last Modified by GUIDE v2.5 02-Jun-2017 13:23:23

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @countGUI62_OpeningFcn, ...
                   'gui_OutputFcn',  @countGUI62_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before countGUI62 is made visible.
function countGUI62_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to countGUI62 (see VARARGIN)

% Choose default command line output for countGUI62
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes countGUI62 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = countGUI62_OutputFcn(hObject, eventdata, handles) 
...
%         [breath, heart]=heartbeat5(RawData);  %%10列+相关
   ...
   [people,r2,thre]=counting62_GUI(RawData);
   set(handles.edit1,'string',num2str(people));   
%    ax=[1:bitch]./Fs1;
   x=0+5/1280:5/1280:5;   
   axes(handles.axes1);
   plot(x,r2(50,:));
%    line([0,5],[T,T],'color','r');
   line([0,5],[thre,thre],'color','r');
   axis([0 5 -2 2]);
   xlabel('距离（m）');
   ylabel('幅度');
   pause(1);
