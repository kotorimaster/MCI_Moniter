function [pp]=newradar_xiuxiuxiu704(RawData,xiu)
M=0;L=0;K=1;
pg=1;rx_num=2;
Batches = 200;
people=0;
...
DDD = PureData(100,:);
ccc =max(PureData(100,101:150))
ccc1 =max(PureData(100,1:100))
if max(DDD) < 5*10^(-4)
    people = 0
elseif max(PureData(100,1:100)) > 2*ccc
    people = 1
elseif max(PureData(100,100:150)) > max(PureData(100,150:200))
     people = 4
else
         people =3
end
...
% [ampmax1(my_batch,1),maxloc1(my_batch,1)]=max(PureData_e(my_batch,1:98));
% [ampmax2(my_batch,1),maxloc2(my_batch,1)]=max(PureData_e(my_batch,36:129));
% [ampmax3(my_batch,1),maxloc3(my_batch,1)]=max(PureData_e(my_batch,115:209));
% [ampmax4(my_batch,1),maxloc4(my_batch,1)]=max(PureData_e(my_batch,100:230));
% [ampmax5(my_batch,1),maxloc5(my_batch,1)]=max(PureData_e(my_batch,131:225));
% %幅值
...
% % variable_meanmean = [Fine_max detail1 detail2 detail3 variable_mean];
% 
