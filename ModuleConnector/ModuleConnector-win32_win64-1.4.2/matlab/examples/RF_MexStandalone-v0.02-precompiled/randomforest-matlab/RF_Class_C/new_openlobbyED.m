function [people input_my]=new_openlobbyED(RawData,xiu)
tic
FrameStitchnum=5;
SpeedOfLight=3*10^8;
Resolution=1/256;
...
for raw=1:1:Batches
 ...
 if raw==1                                                              
    ClutterData(raw,:)=(1-alpha)*BandpassData(raw,:).*wt; 
end
if raw>1
    ClutterData(raw,:)=alpha*ClutterData(raw-1,:)+(1-alpha)*BandpassData(raw,:).*wt;
end 
end
PureData=BandpassData-ClutterData;
%%%%%%%%%%%%%%%%%%%%%%%%%%去噪%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
sigma=1;
PureData_denoise=zeros(Batches,256*FrameStitchnum);
C1 = fdct_wrapping(PureData,1,2);
E = cell(size(C1));
 for s=1:length(C1)
   E{s} = cell(size(C1{s}));
   for w=1:length(C1{s})
...
noise =PureData - PureData_denoise;
%%%%%%%%%%%%%%%%%%%%%%%%%%去噪%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for my_batch = 1:Batches
    input_my =[];
meanmean=mean(PureData(my_batch,:));           %第一个特征
variance=var(PureData(my_batch,:));        %第二个特征   
...
Tobs=16.64;
PureData0 = PureData(my_batch,1:Tobs/ts);
...
%  median_p = floor(median(varia));
%  ccc = 0;
 bbb = 0;
%  for iiii = 1:length(varia)
%      if varia(iiii)==1
%          ccc = ccc +1;
%      end
%  end
for iii= 1:length(varia)
     if varia(iii)==0
       bbb   = bbb +1;
     end
end
 
%  if max(varia) > median_p
%      people = median_p
%  end
%  if max(varia) == median_p
%      people = median_p-1
