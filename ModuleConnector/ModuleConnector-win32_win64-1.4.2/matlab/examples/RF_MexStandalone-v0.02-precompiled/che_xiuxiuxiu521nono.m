function [pp]=che_xiuxiuxiu521nono(RawData7,xiu)
tic
FrameStitchnum=3;
SpeedOfLight=3*10^8;
Resolution=1/256;
...
PureData = BandpassData7;

PureData777 = PureData(11:end,:);
...
e_back(my_batch,1)=sum(PureData_e(:,201:400));
e1(my_batch,1)=sum(PureData_e(:,1:140));
e2(my_batch,1)=sum(PureData_e(:,141:195));
e3(my_batch,1)=sum(PureData_e(:,196:323));
e4(my_batch,1)=sum(PureData_e(:,324:344));
e5(my_batch,1)=sum(PureData_e(:,345:370));
%最大值
[ampmax(my_batch,1),maxloc(my_batch,1)]=max(max(PureDataxiao7,[],1));
[ampmax1(my_batch,1),maxloc1(my_batch,1)]=max(PureData_e(:,1:140));
[ampmax2(my_batch,1),maxloc2(my_batch,1)]=max(PureData_e(:,141:195));
[ampmax3(my_batch,1),maxloc3(my_batch,1)]=max(PureData_e(:,196:323));
[ampmax4(my_batch,1),maxloc4(my_batch,1)]=max(PureData_e(:,324:344));
[ampmax5(my_batch,1),maxloc5(my_batch,1)]=max(PureData_e(:,345:370));
%幅值
amp=mean(PureDataxiao7,1);
ampmean(my_batch,1)=mean(amp);
am_front(my_batch,1)=mean(amp(1,1:200));
am_back(my_batch,1)=mean(amp(1,201:400));
end
